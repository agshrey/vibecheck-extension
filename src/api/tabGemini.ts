...
export async function getGeminiFeedback(code: string, explanation: string, fileName: string, language: string, topContext: string, bottomContext: string) {
    const prompt = `
    You are an expert code reviewer. Your task is to evaluate a piece of code that was Github Copilot autogenerated into the file.
    The user has provided an explanation of the code they autocompleted. Your job is to analyze the code, the explanation, and the surrounding context to provide feedback.
    Write your response in first person, as if you are speaking directly to the user.

    Below is a code snippet that a user is attempting to input into a file. You are given:
        - The name and type of the file ("${fileName}")
        - The programming language ("${language}")
        - The code above the autocompleted code ("${topContext}")
        - The code below the autocompleted code ("${bottomContext}")
